...
void TrainPhysics::Update(clock_t delta_time_ms)
{
    ...
    if (curr_abs_speed > m_max_speed) {
        m_acc_direction = 0;
    }

    m_speed.x = (m_acceleration * m_acc_direction * m_vector.x * delta_time_ms) + m_speed.x;
    m_speed.y = (m_acceleration * m_acc_direction * m_vector.y * delta_time_ms) + m_speed.y;

    m_pos.x = m_pos.x + (m_speed.x * delta_time_ms)
              - (0.5 * m_acceleration * m_acc_direction * m_vector.x * delta_time_ms * delta_time_ms);
    m_pos.y = m_pos.y + (m_speed.y * delta_time_ms)
              - (0.5 * m_acceleration * m_acc_direction * m_vector.y * delta_time_ms * delta_time_ms);
}
